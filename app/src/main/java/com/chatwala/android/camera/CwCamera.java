...
public class CwCamera {
    ...
    public boolean startRecording(long maxRecordingMillis, MediaRecorder.OnInfoListener infoListener) {
        if(!hasError()) {
            try {
                //TODO it should never happen, but what if we go above Integer.MAX_VALUE?
                recorder.setMaxDuration((int)maxRecordingMillis);
    ...
    private boolean openCamera() {
        ...
        cameraToggleEnabled = cameraCount >= 2;
        try {
            if(cameraType == CameraType.FRONT) {
                if(cameraCount > 1) {
    ...
    private boolean setRecorderParams(int width, int height) {
        if(camera != null && recorder != null) {
            try {
                ...
                recordingFile = FileManager.getNewRecordingFile();
